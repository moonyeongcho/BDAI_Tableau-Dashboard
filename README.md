...
## 1. 프로젝트 개요
푸드판다 주문 데이터 기반으로 주문 매출 트렌드와 주요 고객 지표를 한 화면에서 확인할 수 있는 고객 분석 대시보드를 제작했습니다.
목표는 신규 유입이 지속되는 상황에서 충성 고객 유지와 이탈 위험 고객 파악에 도움이 되는 경영용 뷰를 제공하는 것입니다.

## 2. 대시보드에서 답하려는 질문
- 평점이 높은 레스토랑이 실제로 주문도 많이 발생하는가
- 신규 가입 흐름에서 활성 고객과 비활성(이탈) 고객 비중이 어떻게 변하는가
- 성별 고객 구성은 어떠한가
- 도시별로 로열 포인트와 주문 빈도가 높은 고객은 누구인가
- 전체 KPI(총 주문 유저 수, 고객별 평균 평점, 주문 완료율, 평균 주문 빈도)는 어떤 수준인가
...
- Worksheet 3: Gender donut chart
  - 성별 고객 비중 도넛 차트
  - 도넛 내부에 총 고객 수 레이블
  - 성별 레이블 및 비중 표시
...
## 4. 인터랙션
- Navigation bar(좌측): Overview / Customer 버튼으로 대시보드 탐색
- Filters(좌측 패널): 지역, 날짜 연도 필터 연동
- Tooltip / Highlight: 차트별 마우스오버, 선택 시 맥락 정보 확인

## 5. 배포 및 공유 방식
GitHub README에서는 대시보드가 동적으로 실행되지 않기 때문에,
아래 방식으로 공유합니다.
